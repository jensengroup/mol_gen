...
class Node():
  ...
  def fully_expanded(self):
    if len(self.children)>self.state.max_children: print len(self.children),self.state.max_children
...
def EXPAND(node):
  ...
  count = 0
  while new_state in tried_children and count <= new_state.max_children:
    count += 1
